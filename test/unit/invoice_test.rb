...
class InvoiceTest < ActiveSupport::TestCase
  ...
  def test_generate_invoice_number_without_macros
    with_invoice_settings 'invoices_invoice_number_format' => 'Test format' do
      project = Invoice.last.project
      invoice_number = Invoice.generate_invoice_number(project)
      assert_equal 'Test format', invoice_number
    end
  end

  def test_generate_invoice_number_with_macros_with_project
    with_invoice_settings 'invoices_invoice_number_format' => '%%PROJECT_ID%%, %%PROJECT_IDENTIFIER%%, %%MONTHLY_PROJECT_ID%%' do
      project = Invoice.last.project
      invoice_number = Invoice.generate_invoice_number(project)
      m_count = "%03d/#{project.id}" % (Invoice.where(project: project, invoice_date: Time.now.beginning_of_month.utc.change(:hour => 0, :min => 0)..Time.now.end_of_month.utc.change(:hour => 0, :min => 0)).count + 1).to_s
      assert_equal "#{project.id}, #{project.identifier}, #{m_count}", invoice_number
    end
  end
end
